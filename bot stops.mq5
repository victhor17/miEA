...
bool IsTradingAllowed()
{
  ...
  if (isTokyo && InpTradeTokyo)
    return true;
  if (isLondon && InpTradeLondon)
    return true;
  if (isNewYork && InpTradeNewYork)
    return true;
  if (isAsia && InpTradeAsia)
    return true;
...
//+------------------------------------------------------------------+
//| Cierra todas las posiciones y cancela órdenes pendientes         |
//+------------------------------------------------------------------+
void CloseAllAndReset()
{
  // 1. Cerrar posiciones activas primero (de atrás hacia adelante)
  int totalPos = PositionsTotal();
  ...
  // 2. Cancelar órdenes pendientes (de atrás hacia adelante)
  int totalOrders = OrdersTotal();
...
void OnTick()
{
  ...
  if (!gridPlaced)
  {
    if (tradingAllowed)
    {
      // Si hay órdenes pendientes o posiciones, limpiar todo antes de colocar nueva rejilla
      if (CountPendingOrders() > 0 || PositionsTotal() > 0)
      {
        // Cerrar solo posiciones del EA (sin resetear acumuladores)
        int totalPos = PositionsTotal();
        for (int i = totalPos - 1; i >= 0; i--)
        {
          ulong ticket = PositionGetTicket(i);
...
//+------------------------------------------------------------------+
//| Cierra todas las posiciones (función auxiliar para compatibilidad)|
//+------------------------------------------------------------------+
...
void CreatePanel(int panelX, int panelY, int panelWidth, int panelHeight)
{
  ...
  CreateLabel("GridPanel_Title", labelX, yPos, "=== EA Grid Trading v1.99 ===", InpColorTitle, InpFontSizeTitle);
  yPos += rowHeight;
